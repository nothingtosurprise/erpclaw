...
# ---------------------------------------------------------------------------
# setup-web-dashboard — moved to erpclaw-os-engine addon (2026-05-04 split)
# ---------------------------------------------------------------------------
# The function previously here clones erpclaw-web from GitHub, runs npm
# install + build, sets up Python venv for the API, runs deploy/setup.sh,
# and (with sudo) configures nginx + Let's Encrypt. VirusTotal Code Insight
# flagged the bash setup.sh execution + sudo invocations as high-risk.
# Per CLAWHUB_FIX_C_PLAN_2026-05-04 D6, the function moved to the optional
# addon `erpclaw-os-engine` as `os-setup-web-dashboard`, isolating those
# patterns from the foundation skill scan.

def _setup_web_dashboard_moved_stub(args):
    """Replaces the old setup_web_dashboard. Returns a structured migration error."""
    print(json.dumps({
        "status": "error",
        "error": "action 'setup-web-dashboard' moved to erpclaw-os-engine addon (renamed to 'os-setup-web-dashboard')",
